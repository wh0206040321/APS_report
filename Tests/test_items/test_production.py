...
class TestProductionPage:
    ...
    # @pytest.mark.run(order=1)
    def test_production_select2(self, login_to_production):
        driver = login_to_production  # WebDriver 实例
        production = ProductionPage(driver)  # 用 driver 初始化 ProductionPage
        driver.execute_script("document.body.style.zoom='1'")
        production.right_refresh('生产报工')
        production.click_button('//p[text()="工作代码"]/ancestor::div[2]/div[3]//i')
        sleep(1)
        eles = production.get_find_element_xpath(
            '(//div[@class="vxe-pulldown--panel-wrapper"])//label/span').get_attribute(
            "class")
        if eles == "ivu-checkbox ivu-checkbox-checked":
            production.click_button('(//div[@class="vxe-pulldown--panel-wrapper"])//label/span')
            production.click_button('//div[@class="filter-btn-bar"]/button')
        sleep(1)
        production.click_button('//p[text()="工作代码"]/ancestor::div[2]//input')
        eles = production.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
        production.right_refresh('生产报工')
        assert len(eles) == 0
        assert not production.has_fail_message()
    ...
    # @pytest.mark.run(order=1)
    def test_production_select3(self, login_to_production):
        driver = login_to_production  # WebDriver 实例
        production = ProductionPage(driver)  # 用 driver 初始化 ProductionPage
        name = production.get_find_element_xpath(
        ...
        production.hover("包含")
        sleep(1)
        production.select_input(first_char)
        sleep(1)
        eles = production.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
        sleep(1)
    ...
    # @pytest.mark.run(order=1)
    def test_production_select4(self, login_to_production):
        driver = login_to_production  # WebDriver 实例
        production = ProductionPage(driver)  # 用 driver 初始化 ProductionPage
        name = production.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ...
        list_ = [ele.text for ele in eles]
        production.right_refresh('生产报工')
        assert all(str(production).startswith(first_char) for production in list_)
        assert not production.has_fail_message()

    @allure.story("过滤条件查询，设置符合结尾查询成功")
    # @pytest.mark.run(order=1)
    def test_production_select5(self, login_to_production):
        driver = login_to_production  # WebDriver 实例
        production = ProductionPage(driver)  # 用 driver 初始化 ProductionPage
        name = production.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ...
        production.click_button('//p[text()="工作代码"]/ancestor::div[2]/div[3]//i')
        production.hover("符合结尾")
        sleep(1)
        production.select_input(last_char)
        sleep(1)
        eles = production.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
        sleep(1)
        list_ = [ele.text for ele in eles]
        production.right_refresh('生产报工')
        assert all(str(production).endswith(last_char) for production in list_)
        assert not production.has_fail_message()

    @allure.story("清除筛选效果成功")
    # @pytest.mark.run(order=1)
    def test_production_clear(self, login_to_production):
        driver = login_to_production  # WebDriver 实例
        production = ProductionPage(driver)  # 用 driver 初始化 ProductionPage
        name = "3"
        sleep(1)
        ...
        production.hover("包含")
        sleep(1)
        production.select_input(name)
        sleep(1)
        production.click_button('//p[text()="工作代码"]/ancestor::div[2]/div[3]//i')
        production.hover("清除所有筛选条件")
        sleep(1)
        ele = production.get_find_element_xpath('//p[text()="工作代码"]/ancestor::div[2]/div[3]//i').get_attribute(
            "class")
        production.right_refresh('生产报工')
