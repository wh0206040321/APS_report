...
class TestItemPage:
    ...
    def test_item_addcodefail(self, login_to_item):
        ...
        sleep(2)
        self.item.click_button('(//span[text()="zhongjingpeng"])[1]')
    ...
    def test_item_addsuccess(self, login_to_item):
        ...
        adddata2 = self.item.get_find_element_xpath(
            '//tr[./td[3][.//span[text()="111"]]]/td[3]'
        ).text
        adddata3 = self.item.get_find_element_xpath(
            '//tr[./td[4][.//span[text()="666"]]]/td[4]'
        ).text

        assert adddata == "zhongjingpeng" and adddata2 == "111" and adddata3 == "666", f"预期数据是111，实际得到{adddata}"
        assert not self.item.has_fail_message()
    ...
    def test_item_addrepeat(self, login_to_item):
        ...
        self.item.enter_texts("//div[@id='04m2qyfz-z28h']//input", "111")
    ...
    def test_item_editrepeat(self, login_to_item):
        ...
        sleep(2)
        self.item.click_button('(//span[text()="zhongjingpeng"])[4]')
        self.item.click_button('(//button[@type="button"]/span[text()="确定"])[5]')
        # 物料员弹窗
    ...
    def test_item_refreshsuccess(self, login_to_item):
        ...
        # 用户筛选框输入123
        self.item.enter_texts(
            '//span[text()=" 用户"]/ancestor::div[3]//input', "admin"
        )
        self.item.click_ref_button()
