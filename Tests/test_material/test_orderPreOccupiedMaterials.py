...
class TestItemPage:
    ...
    def setup(self, login_to_item):
        ...
        self.custom_xpath_list = [
            f'//div[span[text()=" 自定义文本{i} "]]/following-sibling::div//input'
            for i in range(1, 6)
        ]
        ...
        self.req_input_xpath_list = [
            '//div[span[text()=" 需求来源编码 "]]/following-sibling::div//input',
            '//div[span[text()=" 订单代码 "]]/following-sibling::div//input',
            '//div[span[text()=" 预占料数量 "]]/following-sibling::div//input',
            '//div[span[text()=" 产品编码 "]]/following-sibling::div//i',
            '//div[span[text()=" 物料编码 "]]/following-sibling::div//i',
            '//div[span[text()=" 预占料日期 "]]/following-sibling::div//input',
        ]
        ...
        self.all_input_xpath_list = [
            ...
            '//div[span[text()=" 产品单位 "]]/following-sibling::div//input',
            '//div[span[text()=" 物料单位 "]]/following-sibling::div//input',
            '//div[span[text()=" 仓库编码 "]]/following-sibling::div//input',
            '//div[span[text()=" 库位编码 "]]/following-sibling::div//input',
    ...
    def test_orderPreOccupiedMaterials_addsuccess(self, login_to_item):
        ...
        sleep(1)
        # 批量修改输入框
        self.item.batch_modify_dialog_boxs(self.req_input_xpath_list[3:5], '(//table[@class="vxe-table--body"]//tr[1]/td[2])[last()]')
    ...
    def test_orderPreOccupiedMaterials_addrepeat(self, login_to_item):
        ...
        sleep(1)
        self.item.batch_modify_dialog_boxs(self.req_input_xpath_list[3:5],
    ...
    def test_orderPreOccupiedMaterials_editcodesuccess(self, login_to_item):
        ...
        self.item.get_find_message()
        self.item.wait_for_loading_to_disappear()
        self.item.select_input('需求来源编码', text_str)
        ele = self.item.finds_elements(By.XPATH,
                                       f'//table[@class="vxe-table--body"]//tr/td[2]//span[text()="{text_str}"]')
        assert len(ele) == 1
        assert not self.item.has_fail_message()
    ...
    def test_orderPreOccupiedMaterials_editnamesuccess(self, login_to_item):
        ...
        self.item.get_find_message()
        self.item.wait_for_loading_to_disappear()
        self.item.right_refresh('订单预占料')
        self.item.wait_for_loading_to_disappear()
        num = self.item.go_settings_page()
        sleep(1)
        self.item.select_input('需求来源编码', input_value)
        sleep(1)
    ...
    @allure.story("新增多条数据")
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_adds(self, login_to_item):
        self.item.click_add_button()  # 检查点击添加
        # 输入框要修改的值
        text_str = "1测试数据1"
        date_str = "2025/07/23 00:00:00"
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_select2(self, login_to_item):
        self.item.click_button('//div[div[span[text()=" 需求来源编码"]]]//i[contains(@class,"suffixIcon")]')
        sleep(1)
        ...
        sleep(1)
        self.item.click_button('//div[div[span[text()=" 需求来源编码"]]]//input')
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_select3(self, login_to_item):
        ...
        self.item.hover("包含")
        sleep(1)
        self.item.select_input('需求来源编码', first_char)
        sleep(1)
        eles = self.item.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
        sleep(1)
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_select4(self, login_to_item):
        name = self.item.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ).get_attribute('innerText')
        first_char = name[:1] if name else ""
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_select5(self, login_to_item):
        name = self.item.get_find_element_xpath(
            '//div[@class="vxe-table--body-wrapper body--wrapper"]/table[@class="vxe-table--body"]//tr[2]//td[2]'
        ).get_attribute('innerText')
        ...
        sleep(1)
        self.item.select_input('需求来源编码', last_char)
        sleep(1)
        eles = self.item.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
        sleep(1)
        list_ = [ele.text for ele in eles]
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_clear(self, login_to_item):
        name = "3"
        sleep(1)
        self.item.click_button('//div[div[span[text()=" 需求来源编码"]]]//i[contains(@class,"suffixIcon")]')
        self.item.hover("包含")
        sleep(1)
        self.item.select_input('需求来源编码', name)
        sleep(1)
        self.item.click_button('//div[div[span[text()=" 需求来源编码"]]]//i[contains(@class,"suffixIcon")]')
        self.item.hover("清除所有筛选条件")
        sleep(1)
        ele = self.item.get_find_element_xpath(
            '//div[div[span[text()=" 需求来源编码"]]]//i[contains(@class,"suffixIcon")]').get_attribute(
            "class")
        self.item.right_refresh('订单预占料')
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_ctrlIrepeat(self, login_to_item):
        ...
        ele1 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]').get_attribute(
            "innerText")
        self.item.click_button('//div[@class="vxe-modal--footer"]//span[text()="确定"]')
        message = self.item.get_find_element_xpath('//div[text()=" 记录已存在,请检查！ "]').get_attribute(
            "innerText").strip()
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_ctrlI(self, login_to_item):
        sleep(2)
        self.item.click_button('//table[@class="vxe-table--body"]//tr[2]//td[2]')
        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys('i').key_up(Keys.CONTROL).perform()
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]//input', '1没有数据添加')
        sleep(1)
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_ctrlM(self, login_to_item):
        ...
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[2])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]//input', '1没有数据修改')
        ele1 = self.item.get_find_element_xpath(
            '(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]//input').get_attribute(
            "value")
        self.item.click_button('//div[@class="vxe-modal--footer"]//span[text()="确定"]')
    ...
    @allure.story("模拟多选删除")
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_shiftdel(self, login_to_item):
        self.item.right_refresh('订单预占料')
        ...
        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys('i').key_up(Keys.CONTROL).perform()
        sleep(1)
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]//input', '1没有数据修改1')
        self.item.click_button('(//table[@class="vxe-table--body"]//tr[2]/td[3])[2]')
        self.item.enter_texts('(//table[@class="vxe-table--body"]//tr[2]/td[3])[2]//input', '1没有数据修改12')
        sleep(1)
        ele1 = self.item.get_find_element_xpath(
            '(//table[@class="vxe-table--body"]//tr[1]/td[3])[2]').text
        ...
        ele11 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[1]/td[3])[1]').get_attribute(
            "innerText")
        ele22 = self.item.get_find_element_xpath('(//table[@class="vxe-table--body"]//tr[2]/td[3])[1]').get_attribute(
            "innerText")
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_ctrlC(self, login_to_item):
        self.item.right_refresh('订单预占料')
        self.item.click_button('//table[@class="vxe-table--body"]//tr[2]//td[2]')
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_shift(self, login_to_item):
        elements = ['(//table[@class="vxe-table--body"]//tr[1]//td[1])[2]',
                    '(//table[@class="vxe-table--body"]//tr[2]//td[1])[2]']
        self.item.click_button(elements[0])
        # 第二个单元格 Shift+点击（选择范围）
    ...
    # @pytest.mark.run(order=1)
    def test_orderPreOccupiedMaterials_ctrls(self, login_to_item):
        elements = ['(//table[@class="vxe-table--body"]//tr[1]//td[1])[2]',
                    '(//table[@class="vxe-table--body"]//tr[2]//td[1])[2]']
        self.item.click_button(elements[0])
        # 第二个单元格 Shift+点击（选择范围）
    ...
    def test_orderPreOccupiedMaterials_delsuccess3(self, login_to_item):
        ...
        self.item.wait_for_loading_to_disappear()
        value = ['1测试数据1', '1测试数据2']
        self.item.del_all(value, xpath='//div[div[span[text()=" 需求来源编码"]]]//input')
