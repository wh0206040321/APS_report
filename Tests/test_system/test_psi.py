...
#         assert not psi.has_fail_message()
#
#     @allure.story("保存筛选效果成功")
#     # @pytest.mark.run(order=1)
#     def test_psi_clear1(self, login_to_psi):
#         driver = login_to_psi  # WebDriver 实例
#         psi = PsiPage(driver)  # 用 driver 初始化 PsiPage
#         name = "1测试"
#         psi.click_button('//div[p[text()="PSI名称"]]/following-sibling::div//i')
...
#         message = psi.get_find_message()
#         psi.right_refresh()
#         ele = psi.get_find_element_xpath('//div[p[text()="PSI名称"]]/following-sibling::div//i').get_attribute("class")
#         psi.enter_texts('//div[p[text()="PSI名称"]]/following-sibling::div//input', name)
#         eles = psi.finds_elements(By.XPATH, '//table[@class="vxe-table--body"]//tr//td[2]')
#         sleep(1)
#         list_ = [ele.text for ele in eles]
#         assert all(name in text for text in list_)
#         assert message == "保存成功" and ele == "vxe-icon-funnel suffixIcon activeIcon"
#         assert not psi.has_fail_message()
#
#     @allure.story("清除筛选效果成功")
#     # @pytest.mark.run(order=1)
#     def test_psi_clear2(self, login_to_psi):
#         driver = login_to_psi  # WebDriver 实例
#         psi = PsiPage(driver)  # 用 driver 初始化 PsiPage
#         psi.click_button('//div[p[text()="PSI名称"]]/following-sibling::div//i')
#         psi.hover("包含")
