...
class ItemPage(BasePage):
    ...
    def delete_item(self, material_code):
        ...
        ele = self.get_find_element_xpath('//p[text()="物料代码"]/ancestor::div[2]//input')
    ...
    def select_input_item(self, name):
        """选择输入框."""
    ...
    def select_input_itemgroup(self, name):
        """选择输入框."""
        xpath = '//p[text()="物料组代码"]/ancestor::div[2]//input'
        ele = self.get_find_element_xpath(xpath)
        ele.send_keys(Keys.CONTROL + "a")
        ele.send_keys(Keys.DELETE)
        self.enter_texts(xpath, name)
